...
use icons::{automation_icon, chat_icon, settings_icon, skill_icon, thread_icon, wechat_icon};
use leptos::prelude::*;
use serde_json::json;
...
/// The read-only L6 advisory **flaky-rate** badge `(text, title, warn?)`: the
/// design's "agent wrong vs. test flaky?" signal, folded over the verdict corpus.
/// Calm when nothing flaked (`flaky ✓ none`); amber when some did
/// (`flaky ⚠ N (W‰)`). INV-R1: informational only — the label is explicit that it
/// is advisory, never a gate.
fn flaky_badge(state: &crate::data::FlakySummary) -> (String, String, bool) {
    if state.total_checks == 0 || state.flaky_checks == 0 {
        (
            "flaky ✓ none".to_string(),
            "No flaky checks in the verdict corpus; advisory — not a gate".to_string(),
            false,
        )
    } else {
        (
            format!("flaky ⚠ {} ({}‰)", state.flaky_checks, state.worst_permille),
            format!(
                "{} of {} checks flaky in the verdict corpus (max {}‰); advisory — not a gate",
                state.flaky_checks, state.total_checks, state.worst_permille
            ),
            true,
        )
    }
...
pub(crate) fn Sidebar(
    ...
    flaky_summary: RwSignal<Option<crate::data::FlakySummary>>,
    busy: Signal<bool>,
) -> impl IntoView {
    ...
    view! {
        <aside id="sidebar-panel" class="sidebar">
            ...
            <div class="status-row" role="status" aria-live="polite" aria-label=move || if busy.get() { "Runtime running" } else { "Runtime ready" }>
                ...
                // Read-only L6 advisory flaky-rate badge: the design's "agent wrong
                // vs. test flaky?" calibration, informational (INV-R1), never a gate.
                {move || flaky_summary.get().map(|state| {
                    let (text, aria, warn) = flaky_badge(&state);
                    view! {
                        <span class="flaky-badge" class:warn=warn
                            role="status" title=aria.clone() aria-label=aria>{text}</span>
                    }
                })}
            </div>
...
mod tests {
    use super::{
        close_command_kind, flaky_badge, ledger_badge, thread_agent_badge, thread_agent_class,
    };
    use crate::data::{FlakySummary, LedgerIntegrity};
    ...
    #[test]
    fn flaky_badge_calm_when_no_flake() {
        let (text, title, warn) = flaky_badge(&FlakySummary {
            flaky_checks: 0,
            total_checks: 7,
            worst_permille: 0,
        });
        assert_eq!(text, "flaky ✓ none");
        assert!(title.contains("advisory — not a gate"));
        assert!(!warn);
    }

    #[test]
    fn flaky_badge_calm_when_empty_corpus() {
        let (text, _title, warn) = flaky_badge(&FlakySummary::default());
        assert_eq!(text, "flaky ✓ none");
        assert!(!warn);
    }

    #[test]
    fn flaky_badge_warns_with_count_and_worst_rate() {
        let (text, title, warn) = flaky_badge(&FlakySummary {
            flaky_checks: 2,
            total_checks: 9,
            worst_permille: 125,
        });
        assert_eq!(text, "flaky ⚠ 2 (125‰)");
        assert!(title.contains("2 of 9 checks flaky"));
        assert!(title.contains("max 125‰"));
        assert!(title.contains("advisory — not a gate"));
        assert!(warn);
    }
}
