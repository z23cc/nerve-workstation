...
//! stays the single dispatcher, routing to the per-family `run_*_command` methods via
//! the command→executor routing table in [`partition`].
...
use partition::{Executor, executor_for};
