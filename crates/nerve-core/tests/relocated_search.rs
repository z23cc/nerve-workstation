//! Relocated provider-dependent search tests (the in-src `search::tests` module
//! kept only the two pure path-scoring/glob tests). These drive
//! `nerve_fs::FsCatalogProvider`, which an in-src `#[cfg(test)]` module can no
//! longer construct (the `dev-dependencies nerve-fs` back-edge would compile
//! `nerve-core` twice). All assertions are byte-for-byte identical to the
//! originals; only `FsCatalogProvider`/`ScanOptions` now come from `nerve-fs`.
...
fn smart_case_regex_search_is_insensitive_until_pattern_has_uppercase() {
    ...
    let insensitive = search_snapshot(&provider, &snapshot, &insensitive).expect("regex search");
    assert_eq!(insensitive.totals.content_matches, 1);
...
// `CancelToken::cancel_after_checks` is a deterministic cancellation-injection
// hook that only exists under `test-internals` (or an in-crate test build), so
// this single test is gated; the other 17 run under a plain `cargo test` too.
