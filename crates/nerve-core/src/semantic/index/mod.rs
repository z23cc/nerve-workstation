...
impl SemanticIndex {
    ...
    pub fn warm<P: CatalogProvider + Sync>(
        ...
        cancel: &CancelToken,
    ) -> Result<SemanticWarmResponse, NerveError> {
    ...
    pub fn purge_cache(&self) -> Result<Option<PathBuf>, NerveError> {
    ...
    ) -> Result<Vec<&'a CatalogEntry>, NerveError> {
    ...
    fn build_from_chunks(
        ...
        chunk_build: ChunkBuild,
        cancel: &CancelToken,
    ) -> Result<BuiltSemanticIndex, NerveError> {
    ...
    fn embed_chunk_texts(&self, texts: &[String]) -> Result<Vec<Vec<f32>>, NerveError> {
        if texts.is_empty() {
            return Ok(Vec::new());
        }
