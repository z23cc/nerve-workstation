    ...
    fn embed(&self, texts: &[String]) -> Result<Vec<Vec<f32>>, NerveError> {
        ...
        if guard.is_none() {
            ...
            *guard = Some(TextEmbedding::try_new(options).map_err(|err| {
                NerveError::Semantic(format!("embedding model init failed: {err}"))
            })?);
        }
        guard
            .as_mut()
            ...
            .embed(texts, None)
            .map_err(|err| NerveError::Semantic(format!("embedding failed: {err}")))
    }
}
...
impl EmbeddingBackend for FastembedEmbeddingBackend {
    ...
    fn embed_query(&self, query: &str) -> Result<Vec<f32>, NerveError> {
        ...
        embeddings.into_iter().next().ok_or_else(|| {
            NerveError::Semantic("embedding backend returned no query vector".into())
        })
    }
}
    ...
    fn rerank(&self, query: &str, documents: &[String]) -> Result<Vec<f32>, NerveError> {
        ...
        if guard.is_none() {
            let options = RerankInitOptions::new(self.model.clone())
                .with_cache_dir(self.cache_dir.clone())
                .with_show_download_progress(false);
            *guard = Some(TextRerank::try_new(options).map_err(|err| {
                NerveError::Semantic(format!("reranker model init failed: {err}"))
            })?);
        }
        let ranked = guard
            .as_mut()
            ...
            .map_err(|err| NerveError::Semantic(format!("rerank failed: {err}")))?;
...
pub(super) fn parse_embedding_model(model: Option<&str>) -> Result<EmbeddingModel, NerveError> {
    match model.unwrap_or("jina-embeddings-v2-base-code") {
        ...
        other => Err(NerveError::Semantic(format!(
            "unsupported embedding model for semantic_search: {other}"
        ))),
    }
}

pub(super) fn parse_reranker_model(model: Option<&str>) -> Result<RerankerModel, NerveError> {
