...
use crate::models::{NerveError, RootRef};
...
impl RootPolicy {
    ...
    pub fn new(paths: Vec<PathBuf>) -> Result<Self, NerveError> {
        if paths.is_empty() {
    ...
    pub fn resolve_allowed(&self, path: &Path) -> Result<PathBuf, NerveError> {
        ...
        let canonical = candidate
            .canonicalize()
            .map_err(|err| NerveError::io(candidate.clone(), err))?;
            ...
            .any(|root| canonical.starts_with(&root.path))
        {
            ...
            Err(NerveError::OutsideRoots(canonical))
        }
    }
    ...
    pub fn resolve_for_write(&self, path: &Path) -> Result<PathBuf, NerveError> {
        if self.roots.is_empty() {
            return Err(NerveError::NoRoots);
        }
        reject_traversal(path)?;
        ...
        while !ancestor.exists() {
            ancestor = ancestor
                .parent()
                .ok_or_else(|| NerveError::OutsideRoots(candidate.clone()))?;
        }
        let canonical_ancestor = ancestor
            .canonicalize()
            .map_err(|err| NerveError::io(ancestor.to_path_buf(), err))?;
        if !canonical_ancestor.is_dir() {
            return Err(NerveError::io(
                canonical_ancestor,
                std::io::Error::new(
        ...
        let remainder = candidate
            ...
            .map_err(|_| NerveError::OutsideRoots(candidate.clone()))?;
            ...
            .any(|root| resolved.starts_with(&root.path))
        {
            ...
            Err(NerveError::OutsideRoots(resolved))
        }
    }
}

fn reject_traversal(path: &Path) -> Result<(), NerveError> {
    if path
        .components()
...
mod tests {
    ...
    fn no_roots_refuses_resolution() {
        ...
        assert!(matches!(err, NerveError::NoRoots));
    }
