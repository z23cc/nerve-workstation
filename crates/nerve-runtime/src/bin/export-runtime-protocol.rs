...
use nerve_runtime::protocol_codegen::{CONSTANTS_PATH, SCHEMA_PATH, constants_json, schema_json};
use std::env;
use std::fs;
...
fn find_repo_root() -> Result<PathBuf> {
    ...
    loop {
        if dir.join("Cargo.toml").exists() && dir.join("crates/nerve-runtime").exists() {
            return Ok(dir);
        }
        if !dir.pop() {
...
fn check_artifacts(root: &Path, artifacts: &[(&str, String)]) -> Result<()> {
    ...
    bail!(
        "runtime protocol Rust artifacts are stale: {}. Run `cargo run -p nerve-runtime --bin export-runtime-protocol`",
        stale.join(", ")
    )
}
